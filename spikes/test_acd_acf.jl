...
function bbob_sp1(func, targetValue, P, nonImprovementBudget = 1, reps = 10, MaxEval = 1e4*P)
  ...
  for r in 1:reps
    tic()
    x, fitnesses[r], nfevals[r] = acf_adaptive_coordinate_descent(func, P, -10.0, 10.0; 
      MaxEval = MaxEval,
...
x, f, es = acf_adaptive_coordinate_descent(rosenbrock, 2, -5.0, 5.0; stopfitness = 1e-10, howOftenUpdateRotation = 1)

x, f, es = acf_adaptive_coordinate_descent(rastrigin, 10, -100.0, 100.0; stopfitness = 1e-8, howOftenUpdateRotation = 1)
