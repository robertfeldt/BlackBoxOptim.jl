...
type FunctionBasedProblem{FS<:FitnessScheme,SS<:SearchSpace,FO} <: OptimizationProblem{FS}
  ...
  function Base.call{FS<:FitnessScheme,SS<:SearchSpace,FO}(::Type{FunctionBasedProblem}, objfunc::Function, name::String, fitness_scheme::FS, ss::SS, opt_value::FO = nothing)
    if FO <: Number
